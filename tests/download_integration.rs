...
async fn hedge_races_duplicate_connection_and_produces_correct_output() {
    ...
    // file, so steal always fails. try_hedge fires when the worker is still running
    // and current_limit > join_set.len() (after any completion and limit doubling).
    // Both connections write the same byte range via write_at (idempotent).
    // The first to finish snaps the original's counter; download completes correctly.
