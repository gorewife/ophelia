...
impl Render for Sidebar {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .when(!self.collapsed, |el| {
                el.child(
                    div().px(px(16.0)).mb(px(18.0)).child(
                        div()
                            .id("add-download-btn")
                            .flex()
                            .items_center()
            ...
            .when(self.collapsed, |el| {
                el.child(
                    div().flex().justify_center().mb(px(18.0)).child(
                        div()
            ...
            // Navigation items
            .child(
                div().px(px(10.0)).flex().flex_col().gap(px(4.0)).children(
                    nav_items
                        .into_iter()
                        .enumerate()
            ...
            // Storage card
            .when(!self.collapsed, |el| {
                el.child(div().p(px(16.0)).child(storage_card(&self.download_dir)))
            })
    }
}
...
fn storage_card(path: &Path) -> gpui::Div {
    ...
    div()
        ...
        // Label
        .child(div().text_sm().text_color(Colors::finished()).child("used"))
        // Progress bar
        .child(
            div()
                .w_full()
